...
function iniciarApp() {
    ...
    if (IS_ADMIN) {
        ...
        mostrarToast(`Sesión iniciada: Administrador`, "fa-user-shield");
    } else {
...
function renderizarTablaSeccion(key) {
    ...
    sec.ordenes.forEach((item) => {
        ...
        html += `<tr id="row-${item.orden}" style="background-color:${getPastelColor(item.comentario)}">
            ...
            <td><input type="text" class="comment-box" list="list-comentarios" placeholder="..." value="${item.comentario || ''}" onchange="guardarDatosNota('${item.orden}', 'comentario', this.value)" ${inputStateSeguimiento}></td>
            <td><input type="text" class="comment-box" list="list-observaciones" placeholder="Añadir comentario..." value="${item.observaciones || ''}" onchange="guardarDatosNota('${item.orden}', 'observaciones', this.value)" ${inputStateComentarios}></td>
            <td><small style="font-weight: bold; color: var(--h-blue);">${item.autor || '-'}</small></td>
