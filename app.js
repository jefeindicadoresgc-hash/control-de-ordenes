...
let USER_ROLE = null; 
let IS_ADMIN = false;
let ALLOWED_SECTIONS = []; // Guarda las letras de los departamentos permitidos
...
function mostrarToast(mensaje, icono = 'fa-check-circle', extraClass = '') {
...
function getPastelColor(texto) {
    ...
    // Genera un tinte muy oscuro y elegante ideal para Dark Mode
    let h = Math.abs(hash) % 360; return `hsl(${h}, 50%, 12%)`; 
}

// ==========================================
// SISTEMA DE ROLES (FASE 2) Y LOGIN
// ==========================================
function verificarPassword() {
    let inputEl = document.getElementById('pass-input');
    ...
    if (input === 'YCA0') {
        ...
        ALLOWED_SECTIONS = ['A', 'S']; // Modificación solo Siniestros
    } 
    else if (input === 'YCR0' || input === 'YCR') {
        USER_ROLE = 'Normales';
        ALLOWED_SECTIONS = ['N']; // Modificación solo Normales
    } 
    else if (input === 'YCD2') {
        USER_ROLE = 'Garantías';
        ALLOWED_SECTIONS = ['G']; // Modificación solo Garantías
    } 
    else if (input === 'YCD0') {
        USER_ROLE = 'Visor';
        ALLOWED_SECTIONS = []; // No puede editar nada
    } 
    else {
        mostrarToast("Contraseña incorrecta. Intenta de nuevo.", "fa-times-circle", "error");
        inputEl.value = ''; // Limpiar casilla al fallar
        return;
    }
...
function iniciarApp() {
    ...
    // Solo el Administrador (YCA0) puede subir el archivo Excel
    if (!IS_ADMIN) {
    ...
    if (USER_ROLE === 'Visor') {
        ...
        mostrarToast(`Sesión iniciada: ${USER_ROLE}`, "fa-user-check");
    }
...
function iniciarConexionNube() {
    database.ref('excel_compartido').on('value', (snapshot) => {
        ...
        if (datosEnNube && datosEnNube.datos_json) {
            ...
            } catch(e) { console.error("Error al leer Excel", e); }
        } else {
            ...
            if(IS_ADMIN) document.getElementById('ultima-carga-fecha').innerText = "Esperando archivo...";
        }
    });
...
function cargarExcelNube(e) {
    ...
    reader.onload = function(e) {
        ...
        document.getElementById('ultima-carga-fecha').innerText = "Subiendo a la nube...";
        
        database.ref('excel_compartido').set({
            fecha_subida: dateStr,
            datos_json: JSON.stringify(json)
        }).then(() => {
            mostrarToast("Archivo cargado exitosamente.", "fa-cloud-upload-alt");
        }).catch((error) => {
            mostrarToast("Error al subir a la nube.", "fa-times", "error");
            console.error(error);
        });
    };
...
window.guardarDatosNota = function(orden, campo, valor) {
    // Validación Doble de Seguridad antes de guardar en Firebase
    let letraOrden = orden.charAt(0).toUpperCase();
    let canEdit = IS_ADMIN || ALLOWED_SECTIONS.includes(letraOrden);

    if (!canEdit) {
        mostrarToast("Sin permisos para modificar este departamento.", "fa-lock", "error");
        renderizarTablaSeccion(letraOrden); // Refresca para limpiar el intento
        return;
    }
...
// ==========================================
// PROCESAMIENTO Y RENDERIZADO (CORE)
// ==========================================
...
function actualizarResumenGlobal(g) {
    ...
    document.getElementById('kpi-money-sub').innerHTML = `<span style="color:var(--red); font-weight:600;">${moneyFormat.format(g.dineroAlert)}</span> en riesgo crítico`;
}
...
function generarTablaEmail(secciones) {
    ...
    keys.forEach(key => {
        ...
        if (sec.ordenes.length > 0) {
            ...
            html += `<tr><td>${sec.titulo}</td><td class="bg-ok-light">${sec.countOk}</td><td class="${estiloWarn}">${sec.countWarn}</td><td class="${estiloAlert}">${sec.countAlert}</td><td style="text-align:right; font-family:'Consolas', monospace;">${moneyFormat.format(sec.total)}</td></tr>`;
        }
    });
    html += `</tbody></table>`;
...
function renderizarTablaSeccion(key) {
    ...
    // Aplicación de Permisos Visuales (Bloqueo UI)
    let canEdit = IS_ADMIN || ALLOWED_SECTIONS.includes(key);
    let inputState = canEdit ? '' : 'readonly disabled style="background:transparent; cursor:not-allowed; border-color:transparent;" title="Bloqueado por permisos"';
...
function aplicarFiltroModal() {
    ...
    chartInstance1 = new Chart(ctx1, { type: 'bar', data: { labels: Object.keys(conteoAsesor), datasets: [{ label: 'Órdenes', data: Object.values(conteoAsesor), backgroundColor: '#2f81f7', borderRadius: 6 }] }, options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, datalabels: { anchor: 'end', align: 'end', color: '#7d8590', font: { weight: 'bold' }, formatter: v => v } }, scales: { x: { display: false } } } });
    ...
    Object.entries(conteoAsesor).sort((a,b)=>b[1]-a[1]).forEach(([a,c]) => { htmlAsesor += `<tr onclick="window.verDetalleFiltrado('asesor', '${a}')"><td>${a}</td><td><span class="count-badge" style="background:var(--accent);">${c}</span></td></tr>`; });
...
function copiarTablaDetalle() { 
    ...
    window.getSelection().removeAllRanges(); window.getSelection().addRange(range); 
    try { document.execCommand("copy"); mostrarToast("¡Lista copiada!"); } catch (err) { mostrarToast("Error al copiar.", "fa-times", "error"); } 
    window.getSelection().removeAllRanges(); 
}

function exportarAExcel() {
    if (!RAW_EXCEL_DATA || RAW_EXCEL_DATA.length === 0) { mostrarToast("No hay datos cargados para exportar.", "fa-exclamation-circle", "error"); return; }
...
function exportarDetalleAExcel() { 
    let tabla = document.getElementById('tabla-detalle-copiar'); if (!tabla) { mostrarToast("No hay datos para exportar.", "fa-exclamation-circle", "error"); return; } 
